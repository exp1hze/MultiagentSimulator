...
int main(int argc, char **argv)
   ...
   ////////////////////////////////////////////////////////////////
   // beginning of file "plot.gnu" that includes all gnuplot commands
   // for plotting data
   ...
   // Also prints to two files:
   //     <prefix>_plot_results.txt aggregates data for <prefix> run
   //                into one line of data.  Can be merged with same
   //                file from all prefixes to plot data across rp values
   //                (with rp values as x-axis and y-axis data averaged
   //                over all runs (with CI).
   //                cat all files into a log file and plot with all.gnu
   //     <prefix>_plot_drop_results.txt prints specifically the data at
   //                each drop, again with rp values on x-axis and y-axis
   //                values averages over all runs (with CI).
   //                cat all files into a log file and plot with all.gnu
...
void get_stepnest_averages(int start_run, int end_run, int max_steps, char prefix[])
 ...
 //  int need_met;
   ...
   before_drop = (average[124]+average[249]+average[374]+average[499])/4.0;
   after_drop = (average[125]+average[250]+average[375]+average[500])/4.0;
   drop = ( (average[249]-average[250])
		+ (average[374]-average[375])
		+ (average[124]-average[125])
		+ (average[499]-average[500]) ) /4.0;
printf(" before %lf after %lf drop %lf\n", before_drop, after_drop, drop);

   time_to_recover = 0;
   i = 125;
   while (average[i] < average[124])  i++;
   time_to_recover += i - 125;
   i = 250;
   while (average[i] < average[249])  i++;
   time_to_recover += i - 250;
   i = 375;
   while (average[i] < average[374])  i++;
   time_to_recover += i - 375;
   i = 500;
   while (average[i] < average[499])  i++;
   time_to_recover += i - 500;
   time_to_recover = time_to_recover/4.0;
   ...
   fscanf(fp, "%s %lf %lf %s %lf %lf %s %lf %lf %s %lf %lf",
	string_closest_food, &avg_closest_food, &ci_closest_food, 
	string_total_food, &avg_total_food, &ci_total_food,
	string_actors, &avg_avg_actors_per_step, &ci_avg_actors_per_step,
	string_need, &avg_steps_need_met, &ci_steps_need_met);
   fclose(fp);
   // then print
   fp = fopen(filename, "w");
   fprintf(fp, " %s ", prefix);
   ...
   // print individual drop info to file for gnuplot
   sprintf(filename, "%s_plot_drop_results.txt", prefix);
   fp = fopen(filename, "w");
   fprintf(fp, " %s ", prefix);
   fprintf(fp, " at125 b,ci %lf %lf a,ci %lf %lf d %lf", 
           average[124], ci[124], average[125], ci[125],
           average[124]-average[125]);
   fprintf(fp, " at250 b,ci %lf %lf a,ci %lf %lf d %lf", 
           average[249], ci[249], average[250], ci[250],
           average[249]-average[250]);
   fprintf(fp, " at375 b,ci %lf %lf a,ci %lf %lf d %lf", 
           average[374], ci[374], average[375], ci[375],
           average[374]-average[375]);
   fprintf(fp, " at500 b,ci %lf %lf a,ci %lf %lf d %lf\n", 
           average[499], ci[499], average[500], ci[500],
           average[499]-average[500]);
   fclose(fp);
...
void get_nest_averages(int start_run, int end_run, int max_steps, int pop_size, char prefix[])
   ...
   z = 1.96;
   ci_closest_food = z * stdev_closest_food/sqrt((double)num_runs);
   ci_total_food = z * stdev_total_food/sqrt((double)num_runs);
   ci_last_actor = z * stdev_last_actor/sqrt((double)num_runs);
   ci_avg_last_actor = z * stdev_avg_last_actor/sqrt((double)num_runs);
   ci_avg_actors_per_step = z* stdev_avg_actors_per_step/sqrt((double)num_runs);
   ci_steps_need_met = z * stdev_steps_need_met/sqrt((double)num_runs);

   // print data
   printf(" Closest food:   avg %lf stdev %lf ci %lf\n", 
		avg_closest_food, stdev_closest_food, ci_closest_food);
   printf(" Total food:     avg %lf stdev %lf ci %lf\n", 
		avg_total_food, stdev_total_food, ci_total_food);
   printf(" Last actor:     avg %lf stdev %lf ci %lf\n", 
		avg_last_actor, stdev_last_actor, ci_last_actor);
   printf(" Avg last actor: avg %lf stdev %lf ci %lf\n", 
		avg_avg_last_actor, stdev_avg_last_actor, ci_avg_last_actor);
   printf(" Avg actors per step: avg %lf stdev %lf ci %lf\n", 
		avg_avg_actors_per_step, stdev_avg_actors_per_step, ci_avg_actors_per_step);
   printf(" Pct steps need met: avg %lf stdev %lf ci %lf\n", 
		avg_steps_need_met, stdev_steps_need_met, ci_steps_need_met);
   ...
   sprintf(filename, "%s_plot_results.txt", prefix);
   fp = fopen(filename, "w");
   fprintf(fp, " closest_food_av_ci %lf %lf",
		avg_closest_food, ci_closest_food);
   fprintf(fp, " total_food_av_ci %lf %lf", avg_total_food, ci_total_food);
   fprintf(fp, " actors_p_step_av_ci %lf %lf",
		avg_avg_actors_per_step, ci_avg_actors_per_step);
   fprintf(fp, " pct_steps_need_met_av_ci %lf %lf\n",
		avg_steps_need_met, ci_steps_need_met);
   fclose(fp);
